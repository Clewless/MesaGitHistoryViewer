...
def run_diagnostics() -> Dict[str, Any]:
    ...
    print(f"Mesa directory exists: {info['mesa_dir_exists']} (git repo: {info['mesa_is_git_repo']})")
    print("-" * 50 + "\n")
...
def interactive_remediate(info: Dict[str, Any]) -> None:
    ...
    for s in suggestions:
        ...
        if not cmd:
            print(f"No automatic command available for: {desc}. Please follow printed instructions.")
            continue
...
class MesaViewerApp:
    def __init__(self, root: tk.Tk) -> None:
        try:
            ...
            if not self.git_available:
                try:
                    messagebox.showwarning("Git not found", "Git is not installed or not in PATH. The 'Refresh' feature will be disabled.")
                except Exception:
                    pass
    ...
    def setup_ui(self) -> None:
        try:
            ...
            self.refresh_btn = ttk.Button(toolbar, text="Refresh Data (Git Pull)", command=self.start_refresh)
            self.refresh_btn.pack(side=tk.LEFT)
            if not getattr(self, 'git_available', True):
                self.refresh_btn.config(state=tk.DISABLED)

            # Download/Update button (redownload the entire Mesa repo)
            self.download_btn = ttk.Button(toolbar, text="Download/Update Mesa Repo", command=self.start_download_mesa)
    ...
    def setup_agg_tab(self) -> None:
        try:
            ...
            months_spin = ttk.Spinbox(controls_frame, from_=1, to=120, textvariable=self.months_var, width=5)
            months_spin.pack(side=tk.LEFT, padx=5)

            ttk.Label(controls_frame, text="months").pack(side=tk.LEFT)

            generate_btn = ttk.Button(controls_frame, text="Generate List", command=self.generate_agg_list)
            generate_btn.pack(side=tk.LEFT, padx=10)

            copy_btn = ttk.Button(controls_frame, text="Copy to Clipboard", command=self.copy_to_clipboard)
            ...
            ttk.Label(self.agg_frame, text="Select text and press Ctrl+C to copy, or Ctrl+A to select all.").pack(side=tk.BOTTOM, pady=5)
        except Exception as e:
    ...
    def show_progress(self) -> None:
        ...
        try:
            if self.root.winfo_exists():
                self.root.after(0, lambda: (self.progress_bar.pack(side=tk.LEFT, padx=10), self.progress_bar.start()))
        except Exception as e:
    ...
    def hide_progress(self) -> None:
        ...
        try:
            if self.root.winfo_exists():
                self.root.after(0, lambda: (self.progress_bar.stop(), self.progress_bar.pack_forget()))
        except Exception as e:
    ...
    def prompt_download_mesa(self) -> None:
        ...
        try:
            ...
            if messagebox.askyesno("Download Mesa Repository", msg):
                ...
                self.log_status("Mesa repository download skipped. App will have limited functionality.")
        except Exception as e:
    ...
    def run_download_mesa_logic(self) -> None:
        ...
        try:
            ...
            try:
                if self.root.winfo_exists():
                    self.root.after(0, lambda: (self.download_btn.config(state=tk.NORMAL), self.refresh_btn.config(state=tk.NORMAL), self.hide_progress()))
            except tk.TclError:
                pass
    ...
    def run_refresh_logic(self) -> None:
        try:
            ...
            try:
                if self.root.winfo_exists():
                    self.root.after(0, lambda: (self.refresh_btn.config(state=tk.NORMAL), self.hide_progress()))
            except tk.TclError:
                pass
                    ...
                    with open(self.changelog_history_file, "r", encoding="utf-8", errors='replace') as f:
                        for line_num, line in enumerate(f, 1):
                            try:
                                ...
                                if match:
                                    self.full_history_data.append((match.group(1), match.group(3), match.group(2)))
                                else:
                                    parts = line.split(" | ", 1)
                                    if len(parts) == 2:
    ...
    def load_summaries(self) -> None:
        ...
        try:
            ...
            try:
                ...
                pass  # release_list may not exist yet during initialization
    ...
    def compute_cutoff_date(months: int, now: Optional[datetime] = None) -> datetime:
        """Compute a cutoff datetime for 'months' ago. Keeps day conservative to avoid invalid dates.

        This helper is deterministic and testable by passing an explicit 'now'."""
        if now is None:
            now = datetime.now()
        ...
        while cutoff_month <= 0:
            cutoff_month += 12
            cutoff_year -= 1
        # Use min day to avoid invalid dates (e.g., Feb 30)
        return datetime(cutoff_year, cutoff_month, min(now.day, 28))
    ...
    def filter_history(self, *args: Any) -> None:
        try:
            ...
            filtered = [
                item for item in self.full_history_data
...
def _print_tk_install_instructions() -> None:
    ...
    if os_name == "Linux":
        ...
        print("On macOS: install Python via Homebrew (brew install python) or ensure your Python distribution includes Tk support.")
        print("If using system Python, you may need to install ActiveTcl or use Homebrew python.")
    elif os_name == "Windows":
        print("On Windows: reinstall Python from python.org and ensure 'Tcl/Tk and IDLE' are selected, or install a Python distribution that bundles Tcl/Tk.")
    else:
...
if __name__ == "__main__":
    ...
    if sys.version_info < (3, 9):
        try:
            messagebox.showwarning("Python version", "This application is tested on Python 3.9+. You are running an older Python version which may cause unexpected issues.")
        except Exception:
            print("WARNING: This application is tested on Python 3.9+. You are running an older Python version which may cause unexpected issues.")
